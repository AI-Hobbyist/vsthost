// CEffect.h : VST2 插件封装（移植自 CVSTHost.h，与多插件宿主解耦）
/******************************************************************************/
#pragma once

#include <windows.h>

#include "aeffectx.h"                   /* VST2 头文件（含 ERect）          */
#include "FxBank.h"

/*===========================================================================*/
/* CEffect : 单个 VST2 效果器对象的封装                                       */
/*===========================================================================*/
class CEffect
{
public:
...
public:
    ...
    bool     bWantMidi;
    bool     bInSetProgram;
    long     nUniqueId;                 /* shell 子插件 uniqueID            */

    HMODULE  hModule;
    char    *sDir;

    audioMasterCallback m_pAudioMaster; /* 宿主回调（Load 时传入）           */

    virtual bool Load(const char *name, audioMasterCallback cb);
    virtual bool Unload();

    virtual bool LoadBank(char *name);
    virtual bool SaveBank(char *name);

    virtual long EffDispatch(long opCode, long index = 0, VstIntPtr value = 0, void *ptr = 0, float opt = 0.f);
    ...
    virtual float EffGetVu() { return (float)EffDispatch(effGetVu) / 32767.f; }
    virtual long EffEditGetRect(ERect **ptr) { return EffDispatch(effEditGetRect, 0, 0, ptr); }
    virtual long EffEditOpen(void *ptr) { long l = EffDispatch(effEditOpen, 0, 0, ptr); bEditOpen = (l > 0); return l; }
