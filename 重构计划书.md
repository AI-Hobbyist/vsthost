...
| **M7 最终交付** ✅ | 应用图标（`icon.png` 去白底 → 全尺寸 `res\icon.ico` 并编入 exe，托盘/标题栏/任务栏同步）、版本信息（**Single VST Host 1.0.0.0**，`vsthost.rc` VERSIONINFO + 关于对话框）、中文说明书 `docs\说明书.md`（随打包内置）、`tools\make_icon.py`、`pack.ps1` 自动附带说明书、`release\` 发布包（x64/x86 宿主 + `shell2vst` 工具 + `wrapper` 模板 + 说明书）、README 中英双语 + 语言切换、`.gitignore`（编译/测试/第三方 SDK/发布产物）、**独立 Shell 拆包工具 `shell2vst.exe`**（`src\tool\` + `shell2vst.vcxproj`，与主程序完全解耦）、代码清理（旧代码移入 `Deprecated\`）、推送 GitHub | 图标/版本/说明书/发布包齐全；shell2vst 独立命令行可用（枚举→生成 exe/dll/vst3 包装器）；双平台 Release 编译通过 |
...
## 14. 最终交付完成记录（2026-08-01）

M1–M7 全部完成并通过双平台（Win32 + x64）Release 编译验证，工作区已推送至 GitHub。

| 项 | 说明 |
|---|---|
| **应用图标** | 根目录 `icon.png` 由 `tools\make_icon.py` 去白底（白度≥235 全透明、200~235 线性过渡）生成全尺寸 `res\icon.ico`（16/24/32/48/64/128/256）；`src\app\vsthost.rc` 以 `IDR_MAINFRAME=128` 编入 exe；主窗口 `OnCreate` 补 `SetIcon`（标题栏/任务栏/Alt-Tab），托盘 `LoadIcon(128)` 同源 |
| **版本信息** | `vsthost.rc` 新增 `VERSIONINFO`：ProductName=Single VST Host、FileVersion/ProductVersion=1.0.0.0；关于对话框同步显示“Single VST Host 1.0” |
| **中文说明书** | `docs\说明书.md`（10 章完整使用手册）；`tools\pack.ps1` 打包时自动把 `docs\*.md` 复制到输出目录（PS 5.1 中文乱码坑：脚本保持纯 ASCII，通配匹配） |
| **独立 Shell 拆包工具** | `shell2vst.exe`（`src\tool\shell2vst_main.cpp` + `shell2vst.vcxproj`）：纯 Win32 控制台、复用 `CSingleHost` 枚举；命令行 `shell2vst.exe <shell> [--out <dir>] [--exe|--dll|--vst3|--all] [--host <exe>]`，按声道子文件夹分类生成 exe/dll/vst3 包装器 + `.ini`；主程序已移除 `--shell2vst`/`--gen-shell-exes`（完全解耦）。**坑**：wprintf 中文输出需 `setlocale(LC_ALL,"")`（默认 C locale 无法转换中文宽字符）；模板查找路径为 `..\..\wrapper\` |
| **发布包** | `release\`（不入库）：`vsthost-x64.exe` / `vsthost-x86.exe` / `shell2vst-x64.exe` / `shell2vst-x86.exe` / `说明书.md` / `wrapper\shell2vst2.dll`、`wrapper\shell2vst3.dll` |
| **文档** | `README.md`（英文，默认显示）+ `README.zh-CN.md`（中文）带语言切换；注明衍生自 Arakula/vsthost（尊重原开发者）与 AI（GitHub Copilot）辅助编写声明；补充开发模板章节 |
| **代码清理** | 旧机架代码统一移入 `Deprecated\`（仅源码，二进制不提交）；`release\`、`Deprecated\Release\` 加入 `.gitignore` |
| **推送** | 规范提交（refactor / feat / docs）推送至 `AI-Hobbyist/vsthost`（origin），`Arakula/vsthost` 保留为 upstream |

---

*本计划书基于对 `vsthost` 源码与 `VST\`（符号链接：`vst2sdk` / `vst3sdk` 3.8.0 / `asio` 2.3 / `JACK2` 1.9.22）的核查编写；实施时以 M1–M7 里程碑推进，每阶段可独立验证。*
